...
function scoreMessage(msg: ArchiveMessage, terms: string[]): number {
...
function inferLocalFolder(manifestPath: string, msg: ArchiveMessage): string {
  const archiveDir = path.dirname(manifestPath);
  const filesDir = path.join(archiveDir, "files");
  if (!fs.existsSync(filesDir)) return "";

  const safe = (value: unknown): string => String(value || "untitled")
    .replace(/[\\/:*?"<>|#%&{}$!'@+=`]/g, " ")
    .replace(/\s+/g, " ")
    .trim()
...
function readManifest(manifestPath: string): ArchiveManifest {
  return JSON.parse(fs.readFileSync(manifestPath, "utf8")) as ArchiveManifest;
}

function buildRows(manifestPath: string, manifest: ArchiveManifest, args: Args): SearchResult[] {
...
function printRows(manifestPath: string, args: Args, rows: SearchResult[]): void {
  if (args.json) {
...
printRows(manifestPath, args, buildRows(manifestPath, manifest, args));
